        ...
        for split_name, split in {"train": dl_train, "test": dl_test}.items():
            for i, batch in tqdm(enumerate(split), total=len(split), desc=f"processing {split_name}"):
                ...
                for img, label, center, description, filename in zip(imgs, labels, centers, descriptions, filenames):
                    ...
                    Path(os.path.join(params["tokenized"], split_name, class_id)).mkdir(parents=True, exist_ok=True)
                    np.save(os.path.join(params["tokenized"], split_name, class_id, "VQ_" + name.replace("svg", "npy")), vq_tokens)
                    np.save(os.path.join(params["tokenized"], split_name, class_id, "TXT_" + name.replace("svg", "npy")), text_tokens)
