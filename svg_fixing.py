...
def min_dist_fix(global_positions: Tensor, method: str = "min_dist_clip", max_dist: float = None,
    ...
    """
    assert method in ["min_dist_clip",
                      "min_dist_interpolate"], 'method must be either "min_dist_clip" or "min_dist_interpolate"'
...
def min_dist_fix_global(global_positions: torch.Tensor, method: str = "min_dist_clip", max_dist: float = None,
                        **kwargs):
    """
...
def min_dist_fix_global2(global_positions: torch.Tensor, method: str = "min_dist_clip", max_dist: float = None,
                         **kwargs):
    """
...
def min_dist_fix2(global_positions: Tensor, method: str = "min_dist_clip", max_dist: float = None,
                  connect_last: bool = True):
    """
    ...
    """
    assert method in ["min_dist_clip",
                      "min_dist_interpolate"], f'method must be in {["min_dist_clip", "min_dist_interpolate"]}'
    interpolated_positions = []
    min_dists = []
    for i, stroke in enumerate(global_positions):
        ...
        if method == "min_dist_clip":
            ...
            interpolated_positions.append(
                torch.stack([new_start_point, middle_point, middle_point, new_end_point]))  # (4, 2)
...
def get_fixed_svg_drawing(bezier_points: Tensor,
    ...
    assert method in ["clip", "interpolate", "min_dist_clip",
                      "min_dist_interpolate"], f'method must be in {["clipped", "interpolated", "min_dist_clip", "min_dist_interpolate"]}'
    ...
    if method == "clip":
        ...
        if global_position_fixing:
            if kwargs.get("v2") is not None and kwargs.get("v2"):
                fixed_global_shapes = min_dist_fix_global2(global_shapes, method="min_dist_clip", max_dist=max_dist,
                                                           **kwargs)
            else:
                fixed_global_shapes = min_dist_fix_global(global_shapes, method="min_dist_clip", max_dist=max_dist,
                                                          **kwargs)
        else:
        ...
        if global_position_fixing:
            if kwargs.get("v2") is not None and kwargs.get("v2"):
                fixed_global_shapes = min_dist_fix_global2(global_shapes, method="min_dist_interpolate",
                                                           max_dist=max_dist, **kwargs)
            else:
                fixed_global_shapes = min_dist_fix_global(global_shapes, method="min_dist_interpolate",
                                                          max_dist=max_dist, **kwargs)
        else:
...
def get_svg_render(bezier_points: Tensor,
                   ...
                   width: int = 480,
                   num_strokes_to_paint: int = 0):
    drawing = shapes_to_drawing(
        calculate_global_positions(bezier_points, padded_individual_max_length, positions)[:, 0], stroke_width, width,
        num_strokes_to_paint=num_strokes_to_paint)
