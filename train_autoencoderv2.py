...
dataset = NewCausalSVGDataModule("/scratch2/moritz_data/causal_google_fonts",
                              train_batch_size=batch_size,
                              val_batch_size=batch_size,
                              width=128,
                              context_length=125,
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        input_absolute_shape_layers, _, input_merged_images, stop_signals, _, _, _ = data
        # images, shape_layers, stop_signals, caption, merged_layers, merged_images = data
        bs, t, c, w, h = input_merged_images.shape
        mask = stop_signals == 0.
        mask = mask.view(*mask.shape, 1, 1, 1)  # ensure broadcasting
        selected_gt_shape_layers = torch.masked_select(input_absolute_shape_layers, mask).view(-1, c, w, h)
        selected_gt_merged_layers = torch.masked_select(input_merged_images, mask).view(-1, c, w, h)
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'googlefonts_autoencoder_epoch_{epoch}.pth'))
    if os.path.exists(os.path.join(OUTPUT_DIR, f'googlefonts_autoencoder_epoch_{epoch-5}.pth')):
        os.remove(os.path.join(OUTPUT_DIR, f'googlefonts_autoencoder_epoch_{epoch-5}.pth'))
...
csv_file.close()
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'googlefonts_autoencoder_final.pth'))